...
export function TrustSection() {
  ...
  useGSAP(
    () => {
      ...
      if (!containerEl || !visualEl) {
        return;
      }

      const ambientGlow = visualEl.querySelector('.ambient-glow');
      const ambientGlowReverse = visualEl.querySelector('.ambient-glow-reverse');
      const textElements = containerEl.querySelectorAll('.trust-text-element');
      const interfaceBg = visualEl.querySelector('.trust-interface-bg');
      const customerMessage = visualEl.querySelector('.message-customer');
      const pipelineLineOne = visualEl.querySelector('.pipeline-line-1');
      const pipelineNodeOne = visualEl.querySelector('.pipeline-node-1');
      const dataCard = visualEl.querySelector('.data-card');
      const dataCardItems = visualEl.querySelectorAll('.data-card-item');
      const pipelineLineTwo = visualEl.querySelector('.pipeline-line-2');
      const pipelineNodeTwo = visualEl.querySelector('.pipeline-node-2');
      const botMessage = visualEl.querySelector('.message-bot');
  ...
  return (
    <section ref={container} className="scroll-mt-24 py-24 md:py-32 relative overflow-hidden" id="usecase">
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start">
        ...
        <div className="order-3 lg:col-start-1 relative z-10">
