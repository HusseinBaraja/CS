...
type ConversationTurnLogger = StructuredLogger;
...
export interface ConversationTurnOptions {
  ...
  logger: ConversationTurnLogger;
  now?: () => number;
}

export interface ConversationTurnContext extends InboundRouteContext {
  outbound: OutboundMessenger;
}

const DEFAULT_CONVERSATION_HISTORY_WINDOW_MESSAGES = 20;

export const createConversationTurnProcessor = (options: ConversationTurnOptions): ((message: NormalizedInboundMessage, context: ConversationTurnContext) => Promise<void>) => {
