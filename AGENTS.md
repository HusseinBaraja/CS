...
- In the current local environment, `bun dev` can also show a Baileys `unexpected error in 'init queries'` with `message: "bad-request"` after the bot reaches `state: "open"`. Treat that as a separate existing startup issue unless it starts breaking session health, causes reconnect churn, or correlates with missing sync-dependent behavior.
